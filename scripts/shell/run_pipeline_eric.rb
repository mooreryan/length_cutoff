...
`time ruby #{working_dir}/scripts/pipeline-ray-with-cleanup.rb -f #{fasta} -o #{outdir} -b #{working_dir}/scripts -t 4 -k #{kmer} --no-kmer-sweep 1> #{outbase}.out.txt 2> #{outbase}.err.txt`
